...
class Instance extends Phaser.Scene {
    ...
    create() {
        ...
        // (Use 'this' instead of 'this' when in a function block)
        ...
        //     if (this.clients[player.playerId].team === 'red') {
        //         this.scores.red += 10;
        //         //console.log(this.scores.red);
        //     } else {
        //         this.scores.blue += 10;
        //         //console.log(this.scores.blue);
        //     }
    ...
    addPlayer = (playerId) => {
        let player = this.physics.add.sprite(this.playerSpawnPoints[0].x, this.playerSpawnPoints[0].y, 'player')
            .setVelocity(0, -5);
        // player.anchor.setTo(.5);
        // player.scale.setTo(this.spriteScale);
        ...
        /* Teh SP34R */
        // player.spear = player.addChild(this.make.sprite(10, 0, 'spear'));
    ...
    addBaddie = (baddieInfo) => {
        // console.log("Creating baddie: " + baddieInfo.id);
        let baddie = this.baddies.create(this.baddieSpawnPoints.x, this.baddieSpawnPoints.y, 'baddie');
    ...
    handlePlayerMovement = (playerId, input) => {
        this.players.getChildren().forEach((player) => {
            if (playerId === player.playerId) {
    ...
    handlePlayerAttack = (playerId, input) => {
        ...
        this.players.getChildren().forEach((player) => {
            if (playerId === player.playerId) {
                ...
                this.game.time.events.add(50, () => {
                    this.game.physics.arcade.overlap(player.spear, this.baddies, this.damage, () => {
